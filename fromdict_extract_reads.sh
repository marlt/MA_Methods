...
# This script shall iterate over all fastq files and load the according NA and bacterial headers:
...
do
    ...
    then
        ...
        ${SCRIPTS}/extract_reads_dicts.py ${file} "${TXT}/${BN}_NA_hit1.txt" "${TXT}/${BN}_bacterial_hit1.txt" "${FASTQ}/extracted/"
    
    elif [[ ${file} =~ ${regex2} ]]         # pair1
    then
        BN=$(basename ${file} _pair1_trimmed.fastq)
        # get the pair1 bacterial and NA reads 
        ${SCRIPTS}/extract_reads_dicts.py ${file} "${TXT}/${BN}_PE_NA_hit1.txt" "${TXT}/${BN}_PE_bacterial_hit1.txt" "${FASTQ}/extracted/"
